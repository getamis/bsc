...
func newDiffLayer(parent layer, root common.Hash, id uint64, block uint64, nodes *nodeSet, states *StateSetWithOrigin) *diffLayer {
	...
	_ = dl.offloadNodeBlobToDB()
	_ = dl.offloadNodeSetToDB()

	runtime.SetFinalizer(dl, func(dl *diffLayer) {
		_ = deleteNodeSet(dl.root)
		_ = deleteNodeBlobs(dl.block, dl.root)
	})
...
func (dl *diffLayer) offloadNodeBlobToDB() error {
	return setNodeBlobs(dl.block, dl.root, dl.nodes)
}
