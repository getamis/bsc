...
func (h *HashNodeCache) set(root common.Hash, hash common.Hash, node *trienode.Node) {
	if h == nil {
		return
	}
	h.lock.Lock()
	defer h.lock.Unlock()
	_ = setNodeBlob(root, hash, node.Blob)
}
