...
func TestAncientStorage(t *testing.T) {
	...
	// Write and verify the header in the database
	_, err = WriteAncientBlocks(db, []*types.Block{block}, types.EncodeBlockReceiptLists([]types.Receipts{nil}), big.NewInt(100), nil)
...
func TestAncientTransferLogStorageTransferLog(t *testing.T) {
	...
	WriteAncientBlocks(db, []*types.Block{block}, types.EncodeBlockReceiptLists([]types.Receipts{nil}), big.NewInt(100), nil)
	...
	WriteAncientBlocks(db, []*types.Block{block2}, types.EncodeBlockReceiptLists([]types.Receipts{nil}), big.NewInt(101), []*types.TransferLog{})
	tlogs, err = ReadTransferLogs(db, hash, number)
...
func BenchmarkWriteAncientBlocks(b *testing.B) {
	...
	for i := 0; i < b.N; i += batchSize {
		...
		writeSize, err := WriteAncientBlocks(db, blocks, types.EncodeBlockReceiptLists(receipts), td, nil)
		if err != nil {
			b.Fatal(err)
		}
		totalSize += writeSize
...
func TestHeadersRLPStorage(t *testing.T) {
	...
	WriteAncientBlocks(db, chain[:50], types.EncodeBlockReceiptLists(receipts[:50]), big.NewInt(100), nil)
